...
- 🔴 **`courses/pdf/` нет в `main` — мина под продом.** Прод раздаёт PDF курса по весу
  из `/var/www/app/courses/pdf/`, куда файлы положены вручную по SSH. В ветке `main`
  этих файлов нет, а `deploy-main.yml` синхронизирует с `--delete`. **Любой пуш в `main`
  снесёт их и повторно сломает бонусы курса «Управление весом».** Лечится
  cherry-pick коммита `d2588a6` в `main`. Тот же класс, что авария 07.06.
  Плюс ветки разошлись: `dev` впереди `main` на 15, `main` впереди `dev` на 19.
