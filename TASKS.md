...
- **ПРОМОКОДЫ (новый запрос 31.05)** — генерация промокодов в кабинете: партнёр сам задаёт % скидки (в пределах пула), код применяется при оплате. Продумать: где режется скидка (наш чекаут vs нативный промокод Prodamus — рекоменд. наш чекаут), кап = пул (меню ≤50%/600₽, курс ≤4000₽), анти-абьюз (лимит использований/срок). Ждёт подтверждения подхода, входит в этап 3.
- **Этап 2** — редирект `/r/{code}` + клики + кука `.listoshenkov.ru` (Q5 закрыт, разблокировано).
- **Этап 3** — проброс `_param_ref` в Prodamus (3 билдера: preview.html, гайды index.html, лендинги /course/ /zametki/) + атрибуция в вебхуке (резолв identity → orders с buyer-снимком, пул от ФАКТ. суммы, пенсионер −50% не стакается) + применение промокодов.
