...
> **Последнее обновление: 10.08.2026** — курс «Диабет 1 и 2»: упаковка, контент готов. Ранее (30.07): MAX API переезд на `platform-api2.max.ru` + сертификат Минцифры.
>
...
- **Ручной прогон на телефоне** (Telegram и MAX): навигация, «назад», плеер,
  режим «Читать», PDF, deep-link в добавки.
- **Публикация лендинга** в `/var/www/landing/diabet/` — это уже живые продажи.
  Нужна og-картинка (у `/ves/` есть `og.png`, здесь пока нет).
- **`is_active=true`** на продуктах `course_diabetes` / `course_diabetes_pensioner`
  — сейчас `false`, в пейволле кнопка «Скоро в продаже».
- **Тестовая покупка** через Prodamus до включения продаж.

### 🔴 Хвосты
- **Переделать меню под БЖУ 30/30/40.** Алексей поменял соотношение для первого типа
  (в записи звучит 20/40/40, в конспекте помечено как уточнение + пересчёт). Восемь
  готовых меню в бонусах собраны на 20/40/40 и 20/30/50, под новое соотношение не
  пересобраны. Решено переделывать, не помечать как образец.
...
- Точное БЖУ второго типа: на слайде цифры есть, вслух не названы. В конспекте описано
  качественно (перекос в жиры, LCHF).
- **Таймкоды глав пустые.** `timecodes` у всех 28 глав = `[]`, поэтому чипсов
  перемотки под плеером нет. У «Управления весом» их по 10–11 на главу, но там
  главы по 16–19 минут, а здесь средняя 5 минут. Реально нужны только четырём:
  d18 «Палео-протокол» (16:35), d19 «FODMAP» (10:42), d15 «Обязательные
  исследования» (7:54), d16 «Лекарства и побочки» (6:59). Транскрипты без
  таймштампов, для точных меток нужен прогон Whisper по этим четырём файлам.
