...
- **Обновить GitHub PAT** (запланировано на 11.08). Протух, `git push` отдаёт
  `Invalid username or token`, SSH-ключа для GitHub нет. Коммиты лежат локально:
  `d2588a6`, `4f4afee`, `bcca2e9` в `miniapp` + правка этого файла. На VPS хожу по SSH,
  поэтому деплой не заблокирован, но GitHub Actions не работает.
