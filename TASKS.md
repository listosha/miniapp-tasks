...
**▶ Следующий шаг, за Алексеем (отложил 12.09, напоминалка в MAX на 16.09 10:00):**
- [ ] Ключ Yandex Cloud: API-ключ сервисного аккаунта + ID каталога (дать инструкцию на 5 минут;
      тот же ключ включит Wordstat в publisher).
...
Пока только идея, архитектуру не обсуждали. Первые соображения и вопросы к Алексею —
в переписке 12.09; вернуться вместе с агентом видимости (16.09).
