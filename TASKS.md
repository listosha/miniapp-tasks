...
> **Последнее обновление: 26.08.2026** — разобраны выгрузки из ЛК обоих партнёров по БАД: раздел «Волшебные таблетки» почти не конвертит (клики растут, заказы падают). Починены карточки Q10 и индол-3, заведены две новые позиции (ждут вычитки). Письмо в Anagran по интеграции отправлено. Ранее 25.08: промокод LIST переведён с 17% на 10% везде.
>
...
- **Заведены две позиции, которых не было вообще**: Фосфатидилсерин 400 мг (id 223, 10 заказов с начала года) и Астаксантин с маслом МСТ (id 224, 4 заказа). Данные с карточек anagran.ru. ⏳ `is_visible=false` — ждут вычитки текстов.
