...
## 📱 УСТАНОВКА ПРИЛОЖЕНИЯ НА ТЕЛЕФОН — 🟡 ПУНКТЫ 1 И 2 НА DEV 06.09.2026
...
### ⏸ Чего ещё не хватает, чтобы писать людям

1. **Пункт 3: подсказка «что нажать».** После переноса человек оказывается
   в браузере — и на iPhone ему никто ничего не предложит: там нужно «Поделиться →
   На экран „Домой"». Нужны два экрана инструкции под iOS и Android. Без этого
   рассылка приведёт людей в браузер и бросит.
2. Текст сообщения и канал (баннер в приложении + посты в каналы).
...
`20260906_web_login_handoff.sql`, nginx `app.listoshenkov.ru`. Коммит `6aab614` в `dev`.
