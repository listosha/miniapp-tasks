...
| # | Что | Блок |
|---|---|---|
| 1 | Загрузка: убрать выбор типа анализа, «Лабораторию» и «Дату сдачи» | интерфейс |
| 2 | Удаление неудачного фото до отправки | интерфейс |
| 3 | Плашка «пришлите ссылку» (Яндекс.Диск / Google Диск) + поле | интерфейс |
...
| 10 | Интерфейс материалов: 61 запись в `private_content` уже есть, витрина отключена | материалы |
| 11 | Видео материалов: проверить `content_url`, при отсутствии — RuTube или VK Видео | материалы |
| 12 | **Ярлык на рабочий стол (PWA)** — iPhone и Android без магазинов | ядро |
| 12а | → сначала свой вход (magic-link): без него ярлык однажды откроется в пустоту (iOS чистит localStorage через 7 дней) | ядро |
| 12б | → манифест + apple-метатеги + service worker (кэш ТОЛЬКО оболочки, ответы private-data мимо) | ядро |
