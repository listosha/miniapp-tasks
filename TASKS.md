...
> **Последнее обновление: 18.05.2026 (день)** — SM-08-TUNE-2 закрыт (target_calories на сервере + чистка raw из AI payload). Закрытие открытого пункта из ночного QA. Теперь у Алексея на плечах список ниже.
>
...
## 📋 Задачи для Алексея (на руках, 18.05.2026)

Что требует **твоего участия** прямо сейчас. По приоритету:

### 🔴 Высокий приоритет

**A1. Ручное тестирование dev — визуальные сценарии, не покрытые автотестами**
> 135 автотестов прошли (БД, RPC, EF, RLS, безопасность), но UI/PDF/мобильная адаптация проверяются глазами. Чеклист в разделе «🌙 QA ночная сессия 18.05.2026 → 🟡 Не покрыто автотестами» — таблица с конкретными URL.
> Что делать: `https://dev.listoshenkov.ru/menu/quiz.html` → пройти полностью → корзина → preview → `?test_pay=1` → hold (плавный прогресс?) → result (табы дней, ссылки на БАДы открывают конкретный товар?) → PDF (правая сторона ровная? ссылки кликабельны?) → book.html (заявка → пришло на listosha@list.ru?).
> ~1-2 часа полный обход.

**A2. SM-15 — переписать черновики instruction_text с Claude.ai (партии)**
> Файл с черновиками: `C:\Users\listo\Downloads\protocol_instructions_drafts.md` — 121 текст в моём стиле для всех cache_key.
> План: отдай Claude.ai (Opus/Sonnet) попротокольными партиями. Стилевые правила и порядок партий — в шапке файла. После переписки — пришли мне CC SQL-блок (или просто переписанные тексты по cache_key), залью UPDATE.
> Эффект: вернётся настоящий заголовок «📋 Как пользоваться меню» (сейчас временно показывается explanation_text под заголовком «Про твой протокол»).

**A3. Реальный E2E прогон после SM-08-TUNE-2 — проверка калорийности ±50**
> До SM-08-TUNE-2: target_calories=NULL → AI прикидывал на глаз → меню гуляло ±150-200 ккал.
> После SM-08-TUNE-2: EF считает target по Mifflin-St Jeor + чистит raw из payload Anthropic.
> Что делать: один тестовый платёж через `?test_pay=1` на dev. Проверить в result.html, что итоги дней действительно в коридоре target ±50 ккал. Стоит $0.15.
> Если меню снова гуляет — нужно усиливать промпт (отдельный SM-08-TUNE-3).

### 🟠 Средний приоритет

**A4. Решение по SM-12-EXT — 11 БАДов отсутствуют в каталоге `supplements`**
> Список: Берберин, L-глутамин, Куркумин с пиперином, Витамин C 500-1000мг, Электролиты (натрий/калий/магний), Омега-3 EPA+DHA (в высокой дозировке), B12 метилкобаламин, Омега-3 из водорослей.
> Сейчас они работают через fallback `?section=pills&q=<слово>` — открывают раздел Pills с поиском.
> Решение требуется от тебя: заводить в навигатор (8 новых записей в `supplements`) или оставить как есть.

**A5. SM-MERGE dev → main — после A1, A2, A3**
> Перед мержом убедиться:
> - визуальное тестирование пройдено (A1)
> - реальный E2E подтвердил точность target_calories (A3)
> - инструкции из SM-15 залиты (A2) — опционально, не блокер
> - решить: удалить `supabase/functions/dev-skip-payment/` и блок `if (testPayMode)` в `menu/preview.html`, или оставить (EF защищена origin-check, на проде не сработает)

### 🟢 Низкий приоритет / опционально

**A6. Resend: верификация домена `menu@listoshenkov.ru`**
> Письмо «меню готово» приходит. Но проверить, что для разных email-доменов (gmail, yandex, mail.ru) приходит в inbox, не в спам. Если в спам — поднять DKIM/SPF/DMARC через Resend настройки.

**A7. Расчёт target_calories в `quiz.html` и отображение в preview**
> Сейчас target считается в EF generate-menu при первом вызове. Можно вынести расчёт на клиент (та же формула Mifflin) и показывать на превью-экране «Твоя норма ≈ 1700 ккал/день» — даёт пользователю прозрачность ещё до оплаты.
