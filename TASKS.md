...
> **Последнее обновление: 11.08.2026** — курс «Диабет 1 и 2» продаётся, тестовая покупка прошла. Доставка доступа после оплаты переделана. Ранее (30.07): MAX API переезд на `platform-api2.max.ru` + сертификат Минцифры.
>
...
- **Глюкозная серия разблокирована.** 24 поста написаны, диплинк
  `course-diabet` в блоках 3, 6 и 19 теперь рабочий в проде. Курс держал
  запуск серии, держать больше нечему.
- **Тестовая покупка** через Prodamus — единственное, что осталось непроверенным
  из платёжной цепочки: ссылка собирается верно, но приход вебхука и выдача
  доступа на живой оплате ещё не прогонялись.

### 🔴 Хвосты
- **Переделать меню под БЖУ — объём работы сузился.** Разобрал презентацию курса:
  слайд 38 (первый тип) даёт модифицированное **30/30/40**, слайд 39 (второй тип) —
  **20/40/40**. То есть слайд согласен с правкой Алексея, расходится только
  произнесённое в записи. Восемь готовых меню собраны на 20/40/40 и 20/30/50 —
  под второй тип 20/40/40 подходит как есть, пересобирать нужно вариант под
  первый тип. Решение за Алексеем.
...
- Вкладку «Сосуды и сердце» выкатить в прод после проверки на dev.
...
### ⏳ Открытый вопрос — конверсия ленты
...
Гипотеза: карточка (`_renderFeedPost`, index.html ~4813) — иконка, заголовок и
тема, без картинки и текста, зацепиться не за что. **Решено: добавить подводку
из первых строк поста и сравнить отношение `post_view` к сессиям до и после.**
