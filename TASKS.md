...
> **Последнее обновление: 19.05.2026 (ночь)** — двойная ночная сессия 18→19.05. Полный переход на систему рецептов (SM-17 + SM-17-TUNE), magic-link ЛК, БАДы по реальному каталогу, динамический PDF, обновление инструкций.
>
...
## 🚧 ОТКРЫТО НА 19.05.2026 (на первом месте)

Что требует **участия Алексея** прямо сейчас. По приоритету:

### 🔴 Высокий приоритет — блокеры прода

**B1. Реальный обход dev на «mama-test» (60-90 мин)**
> Главная цель: убедиться что после SM-17 (рецепты) меню читается как нормальная еда, а не «насыпь и подай».
> Шаги:
> 1. `https://listoshenkov.ru/menu/` → «Попробовать» → попадаешь на dev/menu/quiz.html
> 2. Пройди квиз → корзина → preview → `?test_pay=1` → hold → result
> 3. На result.html проверь:
>    - блок «📋 Как пользоваться меню» — три уровня (универсальный про белок/жиры + базовые правила + персональная часть)
>    - каждый приём = карточка-рецепт с названием блюда, граммовкой, временем готовки, БЖУ, составом, спойлером «Как готовить»
>    - БАДы — реальные позиции из артлайф/анагран/озон по «Заказать →»
>    - кнопка «🔄 Сделать ещё меню» → чистый квиз
> 4. PDF — скачай и открой на компе + телефоне. Проверь что каждый день на своей странице, шрифты чёткие, ссылки кликабельны.
> 5. Email «🍽 Твои меню в ЛК» — открыть в почте, кликнуть → попадаешь в app/?section=menu со списком меню.
> 6. В ЛК клик на карточку меню → открывается result.html того же меню.

**B2. Проверка реального калоража в результате**
> После SM-17-TUNE: 3/7 в ±50, 4/7 в ±100, среднее dev 5% (было 31%).
> Что делать: сгенерируй 2-3 меню под разные target (1500 / 1700 / 1900) и зафиксируй сколько дней попали в ±100. Если меньше 5/7 — включаем Уровень 3 (S/M/L портики).
> Где смотреть: «Итого за день» в финальной плашке каждого дня.

**B3. SM-MERGE dev → main — после B1+B2 и явного «ок»**
...
> - menu/result.html полностью переписан (структурный рендер + динамический PDF + 3 уровня инструкций + карточки-рецепты)
...
> - 7 новых миграций (recipes/bads_catalog/menu_lk_tokens/SM-17 RPC и т.п.)
> - 1 новая EF send-menu-link
> - generate-menu полностью переписан под рецепты + пост-валидация
...
**B4. Решение по 11 БАДам отсутствующим в каталоге `supplements`**
> (старая A4, не закрыта) Список: Берберин, L-глутамин, Куркумин с пиперином, Витамин C 500-1000мг, Электролиты, Омега-3 EPA+DHA, B12 метилкобаламин, Омега-3 из водорослей.
> После SM-16-V2 для рекомендаций используется `bads_catalog` (101 позиция артлайф/анагран), эти 11 — отдельный кейс «открыть в Навигаторе». Заводить в `supplements` или нет?

**B5. SM-17-TUNE-3 — S/M/L портики (реактивно)**
> Если B2 покажет <70% дней в ±100 → добавить поле `portion_sizes` (jsonb с S/M/L) к 75 блюдам категорий завтрак/обед/ужин, обновить промпт. Подробности в SM-17-TUNE_TZ.md «Уровень 3».
> Если >70% — не нужно, оставить как есть.
...
**B7. SM-PDF-FINAL** — собрать список мелких замечаний по PDF после реального скачивания (если будут). Возможные направления: компрессия для меньшего веса, артефакт на последней странице, fallback scale=2.5 если на слабых устройствах долго генерится.
