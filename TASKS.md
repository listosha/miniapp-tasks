...
## 🚀 Выкатить `send-guide-pdf` на VPS — В ОЧЕРЕДИ, дедлайн 17.08.2026
...
2. **У всех 8 выпусков «Историй» ноль открытий и ноль кликов** — и это НЕ про
   доставку. Шаблон «Истории (passthrough)» (`templates.id=5`) не содержит ни
   `TrackView`, ни `TrackLink`, то есть пикселя открытий и подмены ссылок в
   письмах просто нет. У майских артлайфовских писем шаблон другой, там
   статистика собиралась. Вывод: **по всей серии «Историй» мы летели вслепую** —
   не знаем ни открытий, ни кликов за полтора месяца.
