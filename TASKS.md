...
## 📧 Инвентаризация базы email (заведено 11.08.2026) — НЕ НАЧАТО

Собрать и проанализировать всё, что есть по адресам. Задача Алексея: понять,
чем мы реально владеем, прежде чем что-то по этой базе отправлять.

**Два источника:**
1. **Email-гейт на лид-магнитах** — таблица `lead_magnet_emails` (собирается
   с 06.06, только web-трафик, в Listmonk намеренно не льётся).
2. **База Артлайфа** — отдельный массив адресов, лежит вне приложения.
   Уточнить у Алексея, где она физически (см. также блок про re-permission
   рассылку Артлайфа).

**Что нужно выяснить по каждой базе:**
- сколько адресов всего, сколько живых после дедупа и нормализации;
- пересечение двух баз между собой и с `users.email` в приложении;
- откуда пришли: по гейту — `guide_slug` и `source` (какой рилс/страница привели);
- какие рассылки по этим адресам уже уходили, когда и с каким результатом
  (Listmonk, Resend, серия «Истории восстановления», письма покупателям);
- у кого есть согласие и в каком виде — по гейту оно пишется с датой,
  по базе Артлайфа надо разобраться отдельно;
- сколько адресов реально пригодны для рассылки, а сколько — балласт.

**На выходе:** сводка цифрами + рекомендация, что с этим делать дальше.
⚠️ Помнить ограничение из корп-исследования: массовая рассылка по базе без
внятного согласия незаконна.
