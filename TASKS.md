...
> **Последнее обновление: 02.09.2026** — «Ближний круг»: нормализатор написания единиц и матчер сделаны. Непроверенных записей 12 вместо 41, пороги критических значений разблокированы. Первое же срабатывание оказалось ложным красным — снято. Абсолютные значения выведены из-под процентных маркеров, у мочи появилось своё место в каталоге и замок. Отчёт: `miniapp/docs/private-units-normalizer-2026-09-02.md`.
>
...
- **Блок 1:** забрать файлы по ссылке на облако (`source_url` пишется, ходить по нему некому);
  приём анализов почтой; ЕМИАС.
...
**Порядок, если брать по одному:** ~~нормализатор единиц~~ (сделан 02.09) → ротация
ключей → забор по ссылке → форма «написать». (~~матчер~~ — тоже сделан 02.09)
