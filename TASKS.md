...
> **Последнее обновление: 28.07.2026** — Двойная оплата гайда (Светлана Харченко): доступ выдан, вскрыта корневая причина — ожидание оплаты жило только в памяти вкладки. Сделан экран «ждём подтверждение» без зависимости от редиректа + алерты по повторной/бесхозной оплате. Ранее (15.06): видимость и аналитика монетизации.
>
...
- ⚠️ Побочная находка: у неё **две личности** в `users` — 715 (MAX, все покупки, живая) и 784 (веб по email `flostis@rambler.ru`, один вход 27.04, покупок нет). Зайдёт с браузера — гайдов не увидит. Слияние блокирует UNIQUE на `users.email`. Решение отложено.
...
- Коммит: dev `4384cb9`. **Хвост: выкатить в прод** — фронт и копию EF `prodamus-webhook` (EF общая для dev и прода, деплоится вручную).
