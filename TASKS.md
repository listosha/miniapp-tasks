...
## ⏳ ЖДЁМ ОТВЕТА ПОДДЕРЖКИ PRODAMUS (обращение отправлено 30.08.2026)

Просьба перевыпустить секретный ключ подписи уведомлений об оплате: в кабинете
`nutriciologist.payform.ru` поле «Секретный ключ» доступно только для чтения.

**Наша сторона уже готова.** `prodamus-webhook` принимает два ключа одновременно
(`PRODAMUS_SECRET` и `PRODAMUS_SECRET_PREV`), поэтому в момент замены ни одна оплата
не потеряется — окна, в котором уведомления отвергаются, не будет.

Когда ответят:
- разрешат своё значение → генерирую ключ, Алексей вставляет, я подставляю на сервер;
- выпустят сами → Алексей кладёт ключ в файл, я подставляю.
Порядок: новый в `PRODAMUS_SECRET`, текущий в `PRODAMUS_SECRET_PREV`, пересоздать
контейнер функций, через сутки убрать `PREV`.

**Почему это важно:** ключ лежит в истории git (5 коммитов) и до 30.08 читался из
`docker-compose.yml` с правами 644. С ним подделывается уведомление «оплачено» —
бесплатный курс или меню плюс запуск генерации за наш счёт.
