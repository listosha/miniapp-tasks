...
**▶ Очередь (просьбы Алексея 12.09):**
- [ ] Память и знание Алексея: кто он, проекты, как с ним работать; сама запоминает новое.
- [ ] Google Календарь: коннектор claude.ai на сервере не виден → служебный аккаунт Google
      `metrika-ga-sync@…`; от Алексея: включить Calendar API и открыть доступ к календарю.
- [ ] Раз в день баланс Yandex Cloud (12.09 — 814 ₽). Нужна роль `billing.accounts.viewer`
      служебному аккаунту — ждёт «да».
- [ ] Агенты пишут Солнышку, а не в MAX; она сводит и шлёт одной сводкой. ⚠️ Срочное
      (караул на оплатах) — сразу; нумерация для «3 да» должна сохраниться.
