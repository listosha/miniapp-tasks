...
> **Последнее обновление: 11.08.2026** — курс «Диабет 1 и 2»: песочница проверок, таймкоды, og-картинка. Ранее (30.07): MAX API переезд на `platform-api2.max.ru` + сертификат Минцифры.
>
...
- **Смержить `dev` → `main`.** Сейчас в проде нет ни раздела `dc*`, ни обложек
  курса (404). До мержа диплинк `course-diabet` из глюкозной серии не сработает.
- **Публикация лендинга** в `/var/www/landing/diabet/` вместе с `og.png`
  **и в тот же заход `is_active=true`** на обоих продуктах. Порядок важен:
  `prodamus-webhook` не смотрит на `is_active` и выдаёт доступ по факту оплаты,
  поэтому опубликованный лендинг уже продаёт, а пейволл в приложении при
  `is_active=false` в это время показывает «Скоро в продаже».
- **Тестовая покупка** через Prodamus до включения продаж.
...
- ⚠️ **Ветки сильно разошлись:** `dev` впереди `main` на 15 коммитов, `main` впереди
  `dev` на 19. Разобрать до того, как выкатывать курс. **Правило:** перед любым пушем
  в `main` прогонять сверку `git ls-tree -r --name-only origin/main` против
  `find /var/www/app -type f` — rsync идёт с `--delete`.
