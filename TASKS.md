...
> **Последнее обновление: 02.09.2026** — «Ближний круг»: нормализатор единиц, матчер и забор файлов по ссылке на облако. Непроверенных записей 12 вместо 41, пороги критических значений разблокированы. Первое же срабатывание оказалось ложным красным — снято. Абсолютные значения выведены из-под процентных маркеров, у мочи появилось своё место в каталоге и замок. Ссылки на Яндекс.Диск и Google Диск теперь разбираются расписанием. Отчёты: `miniapp/docs/private-units-normalizer-2026-09-02.md`, `miniapp/docs/private-cloud-link-fetch.md`.
>
...
- Вне блоков: форма «написать» вместо чата (решено 30.08, не сделано — заодно чинит мёртвый
  вызов `add_thread_message` в `profReportIssue`); письма не спрашивают настройки уведомлений;
  вкладки не схлопнуты (7 вместо трёх).

**Порядок, если брать по одному:** ротация ключей Supabase → форма «написать» → приём
анализов почтой. (~~нормализатор единиц~~, ~~матчер~~, ~~забор по ссылке~~ — сделаны 02.09)
