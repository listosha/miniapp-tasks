...
> **Последнее обновление: 09.09.2026 (ночь)** — раздел «Заметки» собран: 10 путеводителей по темам и 27 разборов веществ, всего 37 материалов. **На `dev`, ждёт «ок» на прод.** Отдельно закрыт сетевой вопрос: доказано, что провалы порта 443 — фильтрация по конкретному IP, сервер и Бегет ни при чём; переехали на второй адрес, за 125 замеров новый не падал ни разу.
>
> **Что решается следующим — представление на главной.** Предложено: полка «Что почитать» из трёх карточек вместо ленты постов. Обоснование цифрами: за 30 дней лента показала 47 715 карточек в 2008 сессиях, а постов открыли 131. Это 0,27%, лента крутится вхолостую. Ждёт решения Алексея.
>
