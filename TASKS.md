...
> **Последнее обновление: 31.08.2026** — оба предложения Фиксика приняты и выкачены на dev: схема добавок ведёт прямо к покупке, вернувшимся на входе предлагается консультация. Ждут отмашки на прод. Заодно починена петля обучения агента приложения — она не могла закрыться ни разу.
>
...
## ✅ ДВЕ ПРАВКИ ПО РАЗБОРУ ФИКСИКА (31.08) — на dev, прод ждёт отмашки
...
⏳ **Осталось:** выкатить на прод. На dev проверено вручную — схема, покупка, «как
принимать», копия списка, блок для вернувшегося и его молчание после ответа.
