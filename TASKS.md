...
- 🔑 Перевыпуск ключей Supabase — **инструкция готова**: `miniapp/docs/service-role-rotation-runbook.md`.
  Разблокировано 30.08: подпись сессий отвязана, разлогина не будет. ~час, за один заход.
  Ключи лежат в: `.env`, `docker-compose.yml`, настройках базы (`app.admin_cron_token` — им
  авторизуются ВСЕ 10 заданий pg_cron), `/opt/analytics/.env`, 13 файлах репозитория,
  секретах GitHub Actions.
