...
> **Последнее обновление: 11.08.2026** — курс «Диабет 1 и 2»: ВЫКАЧЕН В ПРОД, лендинг опубликован. Осталось `is_active=true` и тестовая покупка. Ранее (30.07): MAX API переезд на `platform-api2.max.ru` + сертификат Минцифры.
>
...
- 🔴 **`is_active=true`** на `course_diabetes` / `course_diabetes_pensioner`.
  Сейчас рассинхрон: лендинг уже продаёт (webhook не смотрит на `is_active`
  и выдаёт доступ по факту оплаты), а пейволл в приложении пишет «Скоро в продаже».
- **Тестовая покупка** через Prodamus.
...
- 🟠 **Забытый маркер конфликта в `index.html`.** Строка ~1277: `>>>>>>> dev`
  внутри `<div style="display:none" id="navListLegacy">`. На вид и на JS не влияет
  (блок скрыт), но лежит и в проде. Убрать отдельной однострочной правкой.
