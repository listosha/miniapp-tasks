...
> **Последнее обновление: 27.08.2026 (вечер)** — раздел добавок перестроен и в проде, каталог 191 позиция, агент «Поползень» ждёт первого автоматического прогона 28.08 в 08:30. Ничего срочного на Алексее нет.
>
...
## 🎯 Возвратные механики: как «догнать» ушедшего посетителя — 📋 ПЛАН от 26.08.2026

Вопрос Алексея: человек пришёл, посмотрел, ушёл — как догнать его тем, что он смотрел? Может, агент на VPS в онлайн-режиме? Разбор — `Downloads/strategiya-vozvrata-polzovatelya.html`.

**Блокер: догонять нечем.** Из 22 типов событий по добавкам пишется ровно одно — `link_click` (клик по «купить», конец воронки). Открытие карточки не пишется. Фразу «ты смотрел магний» взять неоткуда.

**Вторая дыра:** `index.html:2682` — `sessionId` генерируется заново при каждой загрузке и не сохраняется. Один человек, зашедший дважды, для нас два разных; статистика возвратов по анонимам недостоверна. Чинится одной строкой (`localStorage`). По залогиненным правда другая: из 289 активных за 60 дней **39% возвращались** в разные дни. Каналы: 477 юзеров с telegram_id, 121 с email.

**Три горизонта возврата:**
1. **Внутри сессии** — реагировать сразу (открыл три карточки магния, не выбрал → показать разницу или схему). Лучшее вложение: работает для всех включая анонимов, без логина, бота и согласий, охват в разы больше.
2. **Следующий визит** — «вы смотрели», «ваша схема сохранена». Живёт в `localStorage`, данные не покидают устройство. Сюда же брошенная схема, после появления списка покупок.
3. **Написать в TG/email** — технически готово, упирается в риски.

**💡 Главный вывод: догонять не добавкой, а консультацией.** Баночка за 2 000 ₽ даёт ~200 ₽ бонусами, схема на 5 000 ₽ — ~500 ₽, консультация — всю сумму живыми деньгами. Сигнал, который надо ловить: **повторный интерес к одной теме без покупки**. Такой человек не проваленная продажа БАДа, у него нерешённый вопрос. Мягкая реализация без единого байта на сервере: на третьем заходе в один раздел блок «Не нашли ответ? Разберём вашу ситуацию», счётчик в `localStorage`.

**⚠️ Риски.** 152-ФЗ: запись «интересовался щитовидкой» плюс рассылка на её основе — обработка данных о здоровье, специальная категория, нужно отдельное явное согласие. Горизонты 1–2 безопасны (данные на устройстве), горизонт 3 решать до написания кода. Репутационный: «мы заметили, что вы интересуетесь щитовидкой» читается как слежка. Граница — напоминать о том, что человек сам сделал, а не о том, что мы подсмотрели.

**Про агентов на VPS:** онлайн-отлов избыточен, события копятся в Supabase и не портятся от обработки через час. Агент по расписанию понадобится позже и для другого — собирать сегменты, готовить списки, следить за брошенными схемами. Реалтайм нужен только горизонту 1, а он целиком на фронте.

**Порядок:** починить sessionId + добавить события (открытие карточки, клик по категории, поисковый запрос, пустой результат) → горизонт 1 → избранное для добавок (сейчас его нет вообще) → горизонт 2 → сегмент «повторный интерес без покупки» на консультацию.
...
## 📋 СЛЕДУЮЩЕЕ: стратегия возврата, что осталось

Горизонты 1 и 2 в проде. Осталось то, что упирается не в код.

**Сегмент «повторный интерес без покупки» → консультация.** Сейчас работает грубая
версия: третий день в разделе → предложение разбора. По накопленным событиям можно
сделать точнее — ловить возврат к одной теме (три открытия карточек одной категории
в разные дни, ни одного link_click) и вести на консультацию с темой. Данные для этого
теперь есть: `pills_card_open` с категорией, `vid` для склейки визитов.
💡 Ключевое из разбора: такой человек — не проваленная продажа баночки, а человек
с нерешённым вопросом. Баночка за 2 000 ₽ даёт ~200 ₽ бонусами, консультация — всю
сумму живыми деньгами.

**Горизонт 3 (написать в TG или на почту)** — технически готово (477 юзеров с telegram_id,
бот работает), упирается в 152-ФЗ: «интересовался щитовидкой» + рассылка на этой основе
это обработка данных о здоровье, нужно отдельное явное согласие, а не галочка про cookie.
Решать до написания кода. Плюс репутационный риск: формулировка не должна выдавать тему.

**Замер.** Первый разбор — через 1–2 недели: доходят ли до схем, собирают ли списки,
что ищут и не находят, срабатывают ли подсказки и напоминания. ⚠️ Фильтровать ботов
и наши прогоны: сейчас в событиях в основном они.
...
## ⚠️ ЖДЁТ АЛЕКСЕЯ: отметить позиции в списке — 27.08.2026

Файл с галочками: **`Downloads/anagran-chego-net-vybor.html`**.

65 позиций Anagran, которых у нас нет, с составом, показаниями и ценой, сгруппированы
