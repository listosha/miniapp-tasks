...
### ⏳ Дальше (ждут решений Алексея)

- **Q5** — где редирект `/r/{code}` (рекоменд.: путь на `*.listoshenkov.ru` + кука `.listoshenkov.ru`, не новый сабдомен). → этап 2: редирект + клики + кука.
- **Этап 3** — проброс `_param_ref` в Prodamus + атрибуция в вебхуке (резолв identity → запись в orders). Пилот на меню.
- **Q6** — формат уведомления Prodamus о возврате → сторно комиссии.
- **Q8 (пенсионеры)** — пул считать от ФАКТ. цены (пенсионер −50% → меню 600 → пул 300). Стакать ли партнёрскую скидку с пенсионерской? Рекоменд.: не стакать.
- **Q1 (курс)** — мягкий лид-магнит в заметке. Реальные слаги гормональных заметок для витрины (сейчас placeholder).
