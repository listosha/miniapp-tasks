...
> **Последнее обновление: 30.07.2026** — MAX API: переезд на `platform-api2.max.ru` + сертификат Минцифры (дедлайн MAX был 19.07). Ранее (28.07): двойная оплата гайда и экран ожидания оплаты.
>
...
## 📡 MAX API: переезд на platform-api2.max.ru + CA Минцифры (30.07.2026) — ЗАКРЫТО

Письмо от MAX: до 19.07 перейти с `platform-api.max.ru` на `platform-api2.max.ru`
и добавить сертификат Минцифры как доверенный. Дедлайн был просрочен на 11 дней,
но старый хост на момент работ ещё отвечал — простоя не было.

**Что вскрылось.** Новый хост подписан цепочкой Минцифры (Russian Trusted Root/Sub CA),
которой нет ни в системном trust store VPS, ни в корнях, вкомпилированных в
`supabase/edge-runtime`. Проверено на живом рантайме: edge-runtime **игнорирует и
`DENO_CERT`, и `SSL_CERT_FILE`** (пробовал через монтирование в compose — та же
`UnknownIssuer`). Рабочий путь — `Deno.createHttpClient({caCerts})` прямо в коде.

**Сделано:**
- `supabase/functions/_shared/max-api.ts` — новый общий модуль: база API + `maxFetch()`
  с вшитым PEM Минцифры. Первый общий модуль в этом репо (раньше функции были
  полностью самодостаточными); импорт `../_shared/` в edge-runtime проверен.
- 12 точек вызова в 6 функциях переведены на `maxFetch`: `user-data`, `max-bot-webhook`,
  `publish-post`, `private-admin`, `send-menu-pdf`, `course-data`. Задеплоено на VPS,
  контейнер пересоздан (иначе рантайм отдаёт старый модуль из кэша).
- `auto-publish.yml`: шаг `update-ca-certificates` + fail-fast проверка чейна,
  сертификаты в `.github/certs/`. Механизм проверен на Ubuntu — python/urllib
  начинает доверять новому хосту, дефолтные корни не теряются.
- CA Минцифры поставлен в системный store VPS (`update-ca-certificates`).
- `.gitattributes`: LF для `.crt`/`.pem` — CRLF внутри PEM ломает парсинг.

**Проверено:** `/me` → 200 (бот опознан), `POST /messages` → `dialog.not.found`
(эндпоинт живой), подписка вебхука цела и указывает на `max-bot-webhook`
(`bot_started`, `message_created`) — перерегистрация не нужна. Все 6 функций
стартуют и отдают обычные ошибки валидации, импорт ничего не сломал.

**Фронтенд не затронут:** только `platform-api2.max.ru` под Минцифры. `st.max.ru`
(SDK), `max.ru`, upload/CDN-хосты (`iu/fu/i.oneme.ru`) остались на Let's Encrypt.

**Коммиты:** `465fe03` + `a7bf030` (dev), cherry-pick `e7e205d` + `91a06a0` (main).
В main перенесён только MAX-фикс — остальные 10 коммитов dev ждут релиза.

### ⏳ Хвост
- **Прогон `auto-publish` не подтверждён.** У локального PAT нет scope `workflow`/
  `actions:read`, дёрнуть и прочитать run не получилось. Проверить вкладку Actions
  после очередного тика крона (каждые 15 мин) — шаг «Trust Mincifry CA for MAX API»
  должен быть зелёным.
- **Sub CA Минцифры истекает 06.03.2027.** Обновить PEM в `_shared/max-api.ts`
  и `.github/certs/` заранее. Источник: https://www.gosuslugi.ru/crt
- Старый `platform-api.max.ru` пока жив; когда MAX его погасит — на нашей стороне
  уже ничего не должно сломаться.

---

## 💳 Двойная оплата гайда + экран ожидания оплаты (28.07.2026)

Разбор кейса: Светлана Харченко (`Flostis@rambler.ru`, MAX, `user_id 715`) оплатила гайд «Работа с кишечником» дважды — 25.07 в 19:08 и 19:11, 690 ₽ × 2.

**T-PAY-DUP-001 — выдать доступ по второй оплате** ✅ (боевая БД)
- Техника отработала верно: покупка #36 записалась через секунду после первого чека, второй платёж корректно погашен идемпотентностью (`user_id+product_id+paid`) — дубля в `purchases` нет.
- Ключ к причине: `order_id` у обоих чеков **идентичный** (`guide_kishechnik_tg12345510_1784995503765`). Кнопка «Купить» генерит `order_id` через `Date.now()`, значит второй раз в app она не нажимала — оплатила повторно на той же странице Prodamus, оставшейся открытой во внешнем браузере.
- По решению Алексея вторая оплата закрыта вторым товаром: покупка #37 «Ты в стрессе» (`product_id 10`), привязана к чеку `47041603` и его времени. Возврат не нужен.
- Побочная находка: у неё было **две личности** в `users` — 715 (MAX, все покупки, живая) и 784 (веб по email `flostis@rambler.ru`, один вход 27.04, покупок нет). Зайдя с браузера, она не увидела бы гайдов.

**T-PAY-MERGE-001 — слияние двух личностей** ✅ (боевая БД)
- Инвентаризация по всем 30 таблицам с `user_id`: у 784 нашлось единственное аналитическое событие, больше нигде следов не было.
- Одной транзакцией: событие перенесено на 715, почта освобождена у 784 и проставлена на 715, 784 удалён. Порядок важен — UNIQUE на `users.email` не даёт проставить почту до освобождения.
- Бэкап обеих строк до удаления: `/root/merge_715_784_users_backup.csv` и `..._events_backup.csv` на VPS.
- Итог: 715 = MAX `12345510` + email `flostis@rambler.ru`, 3 покупки, 760 событий. Вход по почте с веба теперь приземляется в тот же аккаунт.

**T-PAY-PENDING-001 — экран «ждём подтверждение оплаты»** ✅ (dev + прод)
- Корень: из TG/MAX WebView Prodamus не возвращает на `urlSuccess`. Поллинг покупки существовал, но жил только в памяти вкладки и был невидим — вернувшись, человек видел прежнюю кнопку «Купить».
- `index.html`: `pending_payment` в localStorage (TTL 30 мин) переживает перезапуск мини-аппа; `resumePendingPayment()` на старте поднимает ожидание и возобновляет поллинг; баннер со спиннером «Ждём подтверждение оплаты… платить второй раз не нужно»; кнопка гайда на время ожидания → «Оплата обрабатывается…»; `buyGuide` блокирует повтор того же слага первые 2 минуты (обходится явным «Оплатить заново»); через 5 минут без подтверждения — предложение написать. Формулировка fallback-баннера обезврежена (был «Не получилось оплатить?» — толкал к повторной оплате тех, кто уже заплатил).
- Попутный баг: `hidePurchaseFallbackBanner()` обнулял `_buyGuidePayUrl` сразу после старта оплаты → кнопка «Открыть оплату здесь» в fallback-модалке была мёртвой.
- `prodamus-webhook`: повторная оплата купленного гайда больше не гасится молча — алерт в TG; «пользователь не найден» в гайд-ветке тоже алертит (orphan-таблицы там, в отличие от курсов, нет).
- RLS: политика `allow_pay_pending_events` (5 новых `purchase_pending_*` / `purchase_repeat_blocked`) применена на живой БД от `supabase_admin`.
- Коммиты: dev `4384cb9`, прод (cherry-pick) `4d1f707`. В прод шёл cherry-pick'ом — `main` отставал от `dev` на 10 коммитов. Перед выкаткой сверил: живой `/var/www/app/index.html` был байт-в-байт равен `origin/main`, расхождения прод↔репо нет.
- EF `prodamus-webhook` выкачена вручную (общая для dev и прода): бэкап `/root/prodamus-webhook-backup-20260728.ts`, живая версия до замены совпадала с репо. Смоук: неподписанный POST → 403 «Invalid signature», функция жива.

---

## 📈 Видимость курсов + аналитика монетизации (15.06.2026)

Сессия по приоритетному ТЗ: курсы открывали 0 раз/нед (нет точки входа), а у топ-разделов монетизация была слепой зоной. Всё задеплоено в dev И прод.

**T-PILLS-001 — трекинг кликов по реф-ссылкам БАДов** ✅ (dev + прод)
- Диагноз: монетизация БАДов = слепая зона. Все 105 видимых БАДов имеют партнёрские ссылки (artlife-msk 56 / anagran 32 / ozon 9 / wb 3 / shop.artlife 3 / apteka 1, промокод LIST / ref-link-code=LIST). Но карточки рендерили `purchase_link` через `linkify()` → обычный `<a target=_blank>` БЕЗ трекинга. `link_click` по БАДам не писался с конца мая (отвалилось при рефакторе на linkify).
- Фикс (`index.html`): `linkifyBuyLinks()` + `suppBuyClick()` — клик пишет `link_click {url, supp_id, source:'pills'}` и открывается через `openExternalLink` (корректно в TG/MAX WebView; добавлен флаг `skipTrack` против задвоения). `link_click` уже был в whitelist — RLS не трогали.
- Коммиты: dev `c498236`, прод (cherry-pick) `e5291b7`.

**T-COURSE-001 — курсы видимы в ленте** ✅ (dev + прод)
- Карточка курса в умной ленте (`_feedTryCourse` + `_FEED_COURSES`, слот 2 round-robin %6, фиолетовая под стиль nav-row-course). Два курса чередуются; купившим не показывается. Это недостающая точка входа в основном потоке.

**T-COURSE-002 — трекинг воронки курсов + RLS** ✅ (события dev+прод, RLS на живой БД)
- Корень: ВСЕ существующие `course_*`/`wc_*` события (course_open, wc_open, course_preview_* …) молча отваливались с запуска курсов — не были в whitelist RLS `analytics_events`. Поэтому в аналитике по курсам был только `section_open`.
- Добавлены события воронки: `course_view {slug, source}` (в openCourseEntry/openWeightCourseEntry, источник параметром — feed/nav), `course_cta_click`, `course_purchase_start` (в courseBuy/wcBuy).
- Миграция `20260615_course_analytics_rls.sql` (в репо `miniapp`): 3 новых + 14 ранее вшитых событий в whitelist. **Применена на живой VPS-БД** от `supabase_admin` (postgres не владелец таблицы), проверена. БД одна на dev/prod.
- Коммиты: dev `e20c994`, прод (cherry-pick) `c81dd86`. ⚠️ В прод шли cherry-pick'ом — dev опережал main на 49 коммитов (незавершённый «Ближний круг» и пр.), мержить всё нельзя.

**T-CONV-C3 — атрибуция ref (закрывает C3 из блока прод-багов)** ✅
- Код-часть (захват `?ref=` в `session_start.event_data`) реализована ранее и РАБОТАЕТ: за 30 дней `landing` (34), `instagram` (31), `landing_course` (7), `pinterest` (3).
- Находка: лендинг уже был размечен `ref=landing` на всех маркетинговых ссылках (дыры нет). 994 web-сессии без ref = прямой/PWA/возвратный трафик, тегами не закрыть.
- Сделано: **гранулярность** на лендинге — `guide/*.html` → `ref=landing_guide`, `free/*.html` → `ref=landing_free`, главная остаётся `ref=landing`. Теперь видно, какой тип контента (SEO-статьи / лид-магниты / главная) приводит в app. Бэкап: `/root/landing-ref-backup-20260615.tgz`. (Хвост: `.md`-двойники для llms.txt ещё с `ref=landing` — косметика, синхронятся при следующем regen.)

---

## 📧 Email-гейт на лид-магнитах — ЗАДЕПЛОЕН В ПРОД (06.06.2026)

Сбор базы email с Instagram-трафика на бесплатных PDF-протоколах — собственный актив для рассылок (без зависимости от площадки). Реализовано по ТЗ от 05.06.

**Что сделано:**
- **Гейтятся 2 PDF** (`GATED_FREE_GUIDES`): «Протокол: железо 8 недель» + «Протокол: волосы 3 месяца». Квиз не трогали.
- **Подача value-first**: показываем первые 2 реальные страницы PDF → градиент-обрыв → форма (email + чекбокс согласия 152-ФЗ со ссылкой на политику). После отправки дорисовываем остаток страниц.
- **Точка входа** — `openFreePdf`/`loadPdf` в `index.html` (единая, ловит deep-link/карточку/share). Рендер вынесен в `renderPdfPageRange`.
- **Только web** (Instagram): TG/MAX-юзеры открывают протоколы без формы (`Platform.platform==='web'` в `freeGuideNeedsGate`). Залогиненным с email гейт тоже не показываем.
- **БД**: таблица `lead_magnet_emails` (миграция `20260606_lead_magnet_email_gate.sql`, применена к боевому бэкенду): email, guide_slug, source=referrer/PARAM (какой рилс привёл), platform, user_id, consent+дата. Запись — anon insert через `sb` + RLS (SELECT закрыт). Выгрузка вручную через psql/service_role.
- **Форма обращается ко всем**: «не потерял(а)», «Согласен(на)».

**Деплой:** dev через git push (`miniapp@dev 1f9b33f`); прод — прямой scp `index.html`+`catalog.json`+обложки в `/var/www/app` (ветка `main` отстаёт от реального прода). Обложки подкастов закоммичены отдельно (`82c3851`).

**Хвосты/решения Алексея:** в Listmonk НЕ передаём (выгрузка ручная); квиз не гейтили.

---

## 🍽 СделайМеню — фикс доставки меню + группировка приёмов (31.05.2026)

Разбор кейса «оплатила меню, в ЛК пусто» (`lmaksimovich@bk.ru`) вскрыл три бага — все исправлены, задеплоены в прод, закоммичены в `miniapp@dev` (`bb9b030`):

1. **ЛК «Мои меню» не работал НИ У КОГО**: `get_my_menus`/`get_menu_by_id` пришпилены к `search_path=public`, зовут `digest()` без схемы, а pgcrypto в схеме `extensions` → RPC падал у всех с релиза SM-17/18. Фикс: `extensions.digest` (миграция `20260531_menu_lk_digest_schema_fix.sql`, применена к боевой БД).
2. **Зависшая генерация**: фоновый job застрял в `generating` (CPU-лимит EF убил `waitUntil`, без алерта). Восстановлено вручную + **корневой фикс** (`miniapp@dev a0268a3`): EF `menu-watchdog` + RPC `claim_stuck_menu_job` + pg_cron каждые 10 мин добивают зависшие job'ы в форграунде (ready+письмо; 3 провала → failed+TG-алерт). Протестирован end-to-end.
3. **Приёмы пищи дробились**: `result.html` рендерил каждое блюдо отдельной карточкой (два «Завтрак» подряд) + промпт плодил лишний «Перекус» при `meal_count=3` и до 4 блюд на приём. Фикс: фронт группирует `meals[]` по `type` в один блок (подытог по приёму); промпт — ровно `meal_count` приёмов, белый список типов в user-сообщении, ≤3 блюда на приём, один `type` на приём.

Меню клиентки перегенерировано новым промптом и записано в ЛК; magic-link на кабинет отправлен; прямое письмо «меню готово» отправлено. Нюанс: первый прогон отдал кеш (`menu_cache` TTL 14 дней, ключ от промпта НЕ зависит) — удалил её строку кеша, перегенерил вживую. День 7 ~1630 ккал (цель 2100) оставили как есть (структура верная, просадка из-за капа 3 блюда). **`menu_cache` очищен полностью** (19 строк) — все новые меню идут с новой структурой.
...
**C3. Атрибуция лендинга — `?ref=landing` ✅ ЗАКРЫТО 15.06.2026** (см. секцию «Видимость курсов + аналитика монетизации» вверху)
> Захват ref в `session_start.event_data` работает (landing/instagram/landing_course пишутся). Лендинг уже размечен ref; добавлена гранулярность guide/free. Остаток null-сессий = прямой/PWA-трафик.
...
_Свободно. SM-10 (Prodamus+webhook) и async-генерация days=7 — сделаны; устойчивость async закрыта сторожем `menu-watchdog` (см. SM-DELIVERY-FIX в «Выполнено»)._
...
#### SM-DELIVERY-FIX: доставка меню — 3 бага + сторож зависших генераций (31.05.2026, PROD)
**Контекст:** Кейс «оплатила меню на 7 дней, пришёл только чек, в ЛК пусто» (`lmaksimovich@bk.ru`). Оплата `paid`, но меню не дошло. Разбор вскрыл ТРИ независимых бага — все исправлены и задеплоены в прод.

**Что сделано:**
- **SM-LK-FIX — ЛК «Мои меню» не работал НИ У КОГО.** `get_my_menus`/`get_menu_by_id` пришпилены к `SET search_path = public` и зовут `digest()` без схемы, а pgcrypto установлен в схему `extensions` → внутри функций `digest(text, unknown) does not exist`, RPC падает, ЛК всегда пуст (с релиза SM-17/18). `send-menu-link` хеширует токен в JS (crypto.subtle), поэтому ВСТАВКА токена шла нормально — ломалась только проверка в RPC. Фикс: квалифицировать `extensions.digest(...)`. Миграция `20260531_menu_lk_digest_schema_fix.sql`, применена к боевой БД. Проверено сквозняком (временный токен → get_my_menus отдаёт меню).
- **SM-GEN-STUCK — зависшая генерация (корневой фикс).** Фоновая генерация в `prodamus-webhook` (`EdgeRuntime.waitUntil`) убивается CPU-лимитом EF на середине → `menu_generation_jobs` застревает в `generating` навсегда (menu_json пуст, ошибки/алерта нет; ЛК и письмо «меню готово» ждут `ready`). Фикс: **EF `menu-watchdog` + RPC `claim_stuck_menu_job` + pg_cron `menu-watchdog` (jobid 8, `6-59/10 * * * *`)** — атомарно (FOR UPDATE SKIP LOCKED) берёт зависший job оплаченного меню (generating>10м / pending>3м), до-генерирует СИНХРОННО в форграунде (полный CPU-бюджет, не waitUntil) → ready+письмо; провал: attempt<3 → pending (повтор), ≥3 → failed+TG-алерт. Один job за прогон (~100с). Протестирован end-to-end. Миграция `20260531_menu_watchdog.sql`, EF `supabase/functions/menu-watchdog/`.
- **SM-MEAL-GROUP — дробление приёмов.** `result.html` рендерил каждое блюдо приёма отдельной карточкой → два «Завтрак» подряд; промпт плодил лишний «Перекус» при `meal_count=3` и до 4 блюд на приём. Фикс фронта: `result.html` группирует `meals[]` по `type` в один блок (заголовок приёма один раз, блюда внутри, подытог по приёму). Фикс промпта `generate-menu`: ровно `meal_count` приёмов, белый список типов в user-сообщении (из ключей `mealTargets`), ≤3 блюда на приём, все блюда приёма — с одним `type`. ⚠️ `menu_cache` (TTL 14 дней) ключ от промпта НЕ зависит → после правки промпта кеш надо чистить, иначе отдаётся старая структура. Кеш очищен полностью (19 строк).

**Восстановление клиентки:** меню перегенерировано новым промптом (3 приёма, без перекуса, ≤3 блюда; день 7 ~1630 ккал при цели 2100 оставлен сознательно) и записано в job; magic-link на ЛК + прямое письмо «меню готово» (прод-ссылка) отправлены (Resend принял, ключ send-only — финальную доставку видно только в дашборде).

**Репо/деплой:** `miniapp@dev` — `bb9b030` (digest+group+prompt), `a0268a3` (watchdog). Прод: `result.html` → `/var/www/app/menu/`, EF `generate-menu`+`menu-watchdog` → общий functions-контейнер (пересоздан), миграции применены к боевой БД. Память: `menu_lk_two_bugs_31_05`.
