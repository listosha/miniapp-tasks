...
2. **Зависшая генерация**: фоновый job застрял в `generating` (CPU-лимит EF убил `waitUntil`, без алерта). Восстановлено вручную (синхронный вызов `generate-menu` → запись в job).
...
Меню клиентки перегенерировано новым промптом и записано в ЛК; magic-link на кабинет отправлен; прямое письмо «меню готово» отправлено. Нюанс: первый прогон отдал кеш (`menu_cache` TTL 14 дней, ключ от промпта НЕ зависит) — удалил её строку кеша, перегенерил вживую. Хвост: день 7 ~1630 ккал (цель 2100) — структура верная, калораж на одном дне просел из-за капа 3 блюда. Старый кеш (до 14 дней) ещё может отдавать прежнюю структуру.
