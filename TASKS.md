...
> **Последнее обновление: 01.09.2026** — покупательница курса «Диабет» не смогла войти: доступ цел, теряется сессия в браузере. Ссылка ей выслана, поставлена задача «Вход в курс должен быть устойчивее».
>
...
- **Блок 2:** ⚠️ **нормализатор написания единиц идёт ПЕРВЫМ.** Измерено на прогоне порогов:
  24 записи из 42 непроверенных отвалились из-за орфографии единиц (`10^9/л` / `10*9/л` /
  `× 10⁹/л`, `ЕД/л` / `МЕ/л` / `Ед/л`), ещё 3 требуют пересчёта. Он же чинит светофор с
  «0 % ↓» на схлопнутых нейтрофилах. Пороги готовы, но включать раньше него нельзя.
  Дальше: отдельный API-ключ из Console, расчёт российской модели.
...
**Порядок, если брать по одному:** нормализатор единиц → ротация ключей → забор по ссылке →
форма «написать».
