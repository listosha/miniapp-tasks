...
**Безопасность**
- ⏳ `PRODAMUS_SECRET` — ждём поддержку Prodamus (см. блок ниже).
...
⚠️ **31.08 УБРАТЬ `PRODAMUS_SECRET_PREV`.** Пока он на месте, старый ключ — тот самый,
что лежит в истории git и ради которого всё затевалось, — продолжает приниматься.
Делается в две строки: убрать переменную из `.env` и `docker-compose.yml`, пересоздать
контейнер функций.
