...
> **Последнее обновление: 11.08.2026** — курс «Диабет 1 и 2»: **ПРОДАЁТСЯ**. Осталась тестовая покупка и прогон на телефоне. Ранее (30.07): MAX API переезд на `platform-api2.max.ru` + сертификат Минцифры.
>
...
### ⏳ Дальше — нужны решения Алексея
- **Ручной прогон на телефоне** (Telegram и MAX). Всё, что можно проверить
  скриптами, уже гоняет `qa/`. Руками остаётся то, что автотест не видит:
  звук при сворачивании плеера, аппаратная «назад» внутри клиентов TG и MAX,
  перемотка по чипсам таймкодов, вид на реальном экране, верность цифр по сути.
