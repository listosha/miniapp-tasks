...
> **Последнее обновление: 13.09.2026 (утро)** — Солнышко присылает утреннюю сводку в 8:30. Утром Алексей заметил, что агенты отписались, а она нет: служба была жива, напоминание в 10:20 пришло вовремя, просто утреннего ритуала у неё не было. Первая сводка ушла 13.09 в 10:43.
>
...
- [x] **Баланс Yandex Cloud раз в день** с 8:00 (было с 9:00, сдвинуто под сводку): пишет только когда денег меньше 300 ₽ или
      ...
      Алексея. 13.09 — 812 ₽.
- [ ] **Google Календарь:** API включён, код готов и сам включится, когда появится доступ.
      На 13.09 ночью служебный аккаунт календарь `listosha@gmail.com` не видит (404), хотя
      Алексей открывал доступ. Проверить: тот ли аккаунт Google, тот ли адрес, право «Внесение
      изменений в мероприятия». Если календарь в другом аккаунте — ID в `/opt/solnyshko/calendar-id`.
- [ ] **Агенты пишут Солнышку** — первым пунктом плана. Ночью без присмотра не трогал: утром
      агенты шлют разборы, сломать их без Алексея на связи нельзя. По одному агенту, с проверкой;
      срочное сразу, номера «3 да» сохраняются.
