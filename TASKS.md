...
### ⏳ Дальше
- блок экшенов `dc_*` в EF `course-data`, строка в роутер `prodamus-webhook`;
- модуль `dc*` в `index.html` (~340 строк по образцу `wc*`), события в allowlist аналитики;
- лендинг `/diabet/` по шаблону `/ves/`, **завести в git** (лендинги курсов лежат вне репо);
- тест на dev → прод.
