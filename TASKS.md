...
> **Последнее обновление: 13.08.2026** — починен MAX лаб-коллектор: терял файлы из медиа-групп (гонка при параллельных вебхуках). Ранее 11.08: первая живая покупка курса «Диабет» прошла и проверена по всей цепочке; в ленту добавлена подводка из первых строк поста (на dev, ждёт «ок» в прод).
>
